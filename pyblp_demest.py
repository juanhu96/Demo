...
if re_estimate:
    with pyblp.parallel(poolnum):
        res = problem.solve(sigma = 0.1, optimization=pyblp.Optimization('trust-constr'))
...
for i in range(len(res.beta)):
    print(res.beta_labels[i], res.beta[i])
print('sigma:', res.sigma)
# TODO: check if we have dist in X1 in the PE paper


de = res.compute_elasticities('dist')
# summary stats
print("Summary stats of elasticities")
