...
#         # output_filename="output_log/Pharmacy/Pharmacy_eval_10000_5_4q_mnl_A${add_num}_random${random_num}.txt" # MIP
...
for add in "${add_values[@]}"; do
  for random in "${random_values[@]}"; do
        ...
        add_num=$(echo $add | grep -o -E '[0-9]+')
        random_num=$(echo $random | grep -o -E '[0-9]+')        
        # output_filename="output_log/Summary/randomization/summary_10000_5_4q_mnl_A${add_num}_random${random_num}_fcfs.txt" # MIP
