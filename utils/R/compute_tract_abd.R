...
VaxDistAbove0.8 = lm(VaxFull ~ pmax(LogDistNearest, log(0.8)), data = MAR01)
...
stargazer(VaxDistAbove0.5, VaxDistAbove0.8, VaxDistAbove1, VaxDistAbove1.6, star.cutoffs = c(0.05, 0.01, 0.001), digits = 3, type = "text", no.space = TRUE, omit.stat = c("ser","f"), dep.var.labels = c("Fully Vaccinated"))
...
MAR01$PredictedVaxFull0.8 <- predict(VaxDistAbove0.8)
...
MAR01$ErrorTerm0.8 <- MAR01$VaxFull - MAR01$PredictedVaxFull0.8
...
MAR01$ABD0.8 <- 0.780 + MAR01$ErrorTerm0.8
MAR01$ABD1 <- 0.788 + MAR01$ErrorTerm1
MAR01$ABD1.6 <- 0.818 + MAR01$ErrorTerm1.6

MAR01 <- MAR01 %>% select(c(Zip, VaxFull, LogDistNearest, PredictedVaxFull, ErrorTerm, ABD, ABD0.5, ABD0.8, ABD1, ABD1.6))
...
TRACT_ABD <- TRACT_ABD %>% group_by(tract) %>% summarize(abd = sum(frac_of_tract_area * ABD),
                                                         ...
                                                         abd0.8 = sum(frac_of_tract_area * ABD0.8),
