...
cap = any(['cap' in arg for arg in sys.argv]) # Argument will be e.g. "cap5" to cap at 5 closest locations
if cap:
    cap_arg = [arg for arg in sys.argv if 'cap' in arg][0]
    cap_thresh = int(cap_arg.replace('cap', ''))
...
setting_tag += f"_cap{cap_thresh}" if cap else ""
...
if flexible_consideration:
    ...
    # NOTE: cap the cap_thresh closest locations
    if cap:
        print(f"Capping the locations at {cap_thresh} closest.")
        print(f"Before capping {len(distdf)}")
        distdf_cap = distdf.groupby('blkid').head(cap_thresh).reset_index(drop=True)
        # both df have the same set of columns, merge on all columns to take intersection
        distdf = pd.merge(distdf, distdf_cap, how='inner')
        print(f"After capping {len(distdf)}")
