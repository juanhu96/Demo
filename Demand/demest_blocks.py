...
setting_tag = f"blk_{nsplits}q{'_reflastq' if ref_lastq else ''}"
...
for qq in range(1, nsplits+1):
    ...
    agent_data_read[f'logdistXhpi_quantile{qq}'] = agent_data_read[f'logdist'] * (agent_data_read['hpi_quantile'] == qq)
...
optimization_config = pyblp.Optimization('trust-constr', {'gtol':1e-8}) #TODO: tighten tolerance
...
for config in [
    ...
    #     if save_to_pipeline:
    #         tract_utils.to_csv(f"{datadir}/Analysis/Demand/_utils.csv", index=False)
